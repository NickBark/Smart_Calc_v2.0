...
void Graph::drawGrid(QPainter* pp) {
  ...
  double x = bc->xMin;
  double x_step = fabs(bc->xMin - bc->xMax) / 32.;
  ...
  step = 20;
  double y = bc->yMin;
  double y_step = fabs(bc->yMin - bc->yMax) / 32.;

  for (int i = 0; i < HEIGHT; i += step, y += y_step) {
    if (!(i % 80) && i != 0) {
      pp->setPen(QColor(0, 0, 0, 255));
      ...
      if (i == HEIGHT / 2)
        ...
        pp->drawText(0, HEIGHT - i, QString::number(y, 'g', 3));
    } else {
      pp->setPen(QColor(0, 0, 0, 100));
      pp->drawLine(0, i, 1000, i);
    }
...
void Graph::drawGraph(QPainter* pp) {
  ...
  for (int i = 0; i < bc->numbOfPoint - 2; i++) {
    //        if (abs(bc->pointArr[i].jy - bc->pointArr[i + 1].jy) >= HEIGHT
    //        / 2)
    //            continue;
