...
void BaseCalc::slotPbResult() {
    ...
    xMin = (double)spbXmin->value();
    xMax = (double)spbXmax->value();
    ...
    if (qStr.size()) {
        ...
        if (wasX) {
            if (diy) {
                ...
                if (fabs(xMax - xMin) > 1000.) h = 0.01;
                if (fabs(xMax - xMin) > 10000.) h = 0.1;
                if (fabs(xMax - xMin) > 100000.) h = 1;
                if (fabs(xMax - xMin) > 500000.) h = 10;
...
void BaseCalc::calcY(char* resStr, Priority* pr, double x, double* y) {
    ...
    if (ret) {
        cg->x.push_back(qQNaN());
        cg->y.push_back(qQNaN());
    } else {
        cg->x.push_back(x);
        cg->y.push_back(*y);
    }
...
void BaseCalc::convertToWindow() {
    ...
    //    for (int i = 1; i < numbOfPoint; i++) {
    //        yNewArr[i] = yNewArr[i - 1] + dy;
    //    }

    //    yNewArr[numbOfPoint - 1] = yMax;
    ...
    for (int i = 0; i < numbOfPoint; i++) {
        ...
        //        //        graph->image.setPixel(pointArr[i].ix,
        //        pointArr[i].jy,
        //        //        qRgb(255, 0, 0));
    }
}

void BaseCalc::diyGraph(char* resStr, Priority* pr, double* result) {
    double x = xMin;
    int ret = 0;
    double h = S21_PI / 180.;
    ...
    //    pointNum = fabs(180 * xMin / S21_PI - 180 * xMax / S21_PI);

    Mx = fabs(180 * xMin / S21_PI - 180 * xMax / S21_PI) / (double)(WIDTH - 1);
    yMin = 0.;
    yMax = 0.;
    ...
    for (int i = 0; i < numbOfPoint; i++) {
        qDebug() << pointArr[i].x << " " << pointArr[i].y;
    }
