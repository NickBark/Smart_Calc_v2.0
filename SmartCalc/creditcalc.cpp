...
void CreditCalc::Designer() {
    ...
    lTime = new QLabel("Срок кредита (месяцы)", this);
...
void CreditCalc::Properties() {
    ...
    lTime->setMinimumSize(150, 25);
    lPersent->setMinimumSize(150, 25);
...
void CreditCalc::Layouts() {
    ...
    glButtons->addWidget(cmType, 0, 0, 1, 2);
    glButtons->addWidget(lSum, 2, 0, 1, 1);
    glButtons->addWidget(dsbSum, 2, 1, 1, 1);
    glButtons->addWidget(lTime, 3, 0, 1, 1);
    glButtons->addWidget(sbTime, 3, 1, 1, 1);
    glButtons->addWidget(lPersent, 4, 0, 1, 1);
    glButtons->addWidget(dsbPersent, 4, 1, 1, 1);
    glButtons->addWidget(pbResult, 5, 0, 1, 1);
...
void CreditCalc::slotPbResult() {
    ...
    if (cr.type == 0) {
        ...
        int dateTmp = QDate::currentDate().month();
